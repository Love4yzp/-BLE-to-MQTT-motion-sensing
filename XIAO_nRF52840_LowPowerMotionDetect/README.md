...
设备启动时自动检测供电方式，行为有所不同：

| 供电方式 | 检测方法 | 行为 |
|----------|----------|------|
| **USB 供电** | VBUS 电压存在 | 不进入睡眠，持续等待运动，支持 AT 命令配置 |
| **电池供电** | VBUS 电压不存在 | 正常睡眠周期，省电模式 |
...
> **提示**：拔掉 USB 后需要重新触发一次运动才能进入电池模式的睡眠周期。
...
通过 USB 连接并使用串口终端可以实时配置传感器参数，无需重新编译固件。

### 使用方法

1. **连接 USB**：将传感器通过 USB 连接到电脑
2. **打开串口终端**：波特率 115200
3. **输入命令**：支持以下 AT 命令
...
| `AT+HELP` | 显示帮助信息 | |
| `AT+INFO` | 显示当前配置和设备信息 | |
| `AT+THRESHOLD=xx` | 设置运动阈值 (hex, 02-3F) | `AT+THRESHOLD=0A` |
| `AT+TAILWINDOW=xxxx` | 设置尾随窗口 (ms, 1000-10000) | `AT+TAILWINDOW=3000` |
| `AT+TXPOWER=x` | 设置发射功率 (dBm, -40 to 4) | `AT+TXPOWER=4` |
| `AT+SAVE` | 保存配置到 Flash（断电保持） | |
| `AT+DEFAULT` | 恢复默认值（不自动保存） | |
| `AT+REBOOT` | 重启设备 | |
...
| 参数 | 范围 | 默认 | 公式/说明 |
|------|------|------|-----------|
| THRESHOLD | 0x02~0x3F | 0x0A | 阈值 = 值 × 31.25mg (0x0A ≈ 312mg) |
| TAILWINDOW | 1000~10000 | 3000 | 广播后等待运动的时间 (ms) |
| TXPOWER | -40 ~ +4 | +4 | BLE 发射功率 (dBm) |

### 注意事项

- **仅 USB 模式可用**：CLI 仅在 USB 供电时激活，电池供电时不响应
- **保存后生效**：`AT+SAVE` 后配置保存到 Flash，下次启动自动加载
- **部分配置需重启**：THRESHOLD 需要重启后才能完全生效（影响 IMU 配置）
- **运动反馈**：USB 模式下每次检测到运动会绿灯闪烁，便于测试阈值
...
| LED | 行为 | 时长 | 含义 |
|-----|------|------|------|
| 绿灯短闪 | 唤醒时 | 30ms | 检测到运动，MCU 启动（电池模式）|
| 绿灯短闪 | 运动时 | 30ms | 检测到运动（USB 模式，每次运动都闪）|
| 蓝灯短闪 | 睡眠前 | 50ms | 即将进入深度睡眠（仅电池模式）|
